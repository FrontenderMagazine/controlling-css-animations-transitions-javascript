...
[Переходы][1] в CSS применяются к элементу, при этом изменение свойства должно происходить 
постепенно на протяжении определённого отрезка времени. 
С [анимациями][2] всё немного по-другому. Они дают более точный контроль над 
...
очень полезную и информативную статью о CSS-переходах, [которую можно найти здесь][5]. 
...
чтобы сделать эти события кросс-браузерными. Вот его полезная 
[статья о перехвате CSS-анимации с помощью JavaScript][6]. А вот [ещё одна][7] о том, как 
...
трансформации элемента так же невозможны, как и в процессе её воспроизведения. 
...
анимации началось бы с текущего состояния. Мы живём в неидеальном мире и в нём 
всё немного сложнее. 
...
Чтобы так сделать, нужно довольно сильно углубиться в код. Придётся покопаться 
...
На этом этапе `keys` является массивом всех ключевых кадров анимации в числовом 
формате.
...
* `transitionEnd` и похожие события достаточно полезны при управлении 
